...
type Rational =
    ...
    static member Normalise (r : Rational) : Rational =
        ...
        let gcd = gcd (Rational.Numerator r) (Rational.Denominator r)
    ...
    member this.Normalise () =
        Rational.Normalise this

    static member Make (num : int) (denom : int) : Rational =
        if denom = 0 then failwith "Invalid zero denominator"
        elif denom < 0 then
            {
                Num = -num
                Denom = (-denom) - 1
    ...
    interface System.IComparable<Rational> with
        member this.CompareTo (other : Rational) =
            ...
            let cmp = Rational.Numerator this * Rational.Denominator other - Rational.Numerator other * Rational.Denominator this
            if cmp < 0 then -1
            elif cmp = 0 then 0
            else 1
    ...
    interface System.IComparable with
        member this.CompareTo (other : obj) =
            match other with
            | :? Rational as other ->
                (this :> System.IComparable<Rational>).CompareTo other
            | _ -> failwith "how?!"
...
module Rational =
    ...
    let add (r1 : Rational) (r2 : Rational) =
        Rational.Make (Rational.Numerator r1 * Rational.Denominator r2 + Rational.Numerator r2 * Rational.Denominator r1) (Rational.Denominator r1 * Rational.Denominator r2)
        |> Rational.Normalise

    let ofInt (i : int) : Rational = { Num = i ; Denom = 0 ; IsNormal = true }

    let times (r1 : Rational) (r2 : Rational) =
        Rational.Make (Rational.Numerator r1 * Rational.Numerator r2) (Rational.Denominator r1 * Rational.Denominator r2)
        |> Rational.Normalise
